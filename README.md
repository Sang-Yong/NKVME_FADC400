...
### 6.1 범용 전하량 스펙트럼 시각화 (`offline_charge.cpp`)
  ...
  * **물리적/교육적 의도:** 아날로그 회로에서 편의상 사용하는 **'펄스 파고(Pulse Height/Amplitude)'** 방법론이 노이즈에 얼마나 취약한지, 그리고 파형 전체의 면적을 구하는 **'전하 스펙트럼(Charge Sum / 펄스 적분)'** 방법론이 잔피크(Fine structures) 보존과 에너지 분해능 면에서 얼마나 압도적으로 우수한지 직관적으로 4분할 캔버스로 비교·검증합니다.
...
### 6.2 PMT 단일 광전자 교정 및 절대 이득 추출 (`offline_spe.cpp`)
  ...
  * **기능 개선 사항:**
      ...
      * **정밀 피팅 엔진:** `Minuit2` 함수 최소화 알고리즘 함수를 도입하여 기초적인 `Fit()` 함수의 수렴 실패 한계를 돌파했습니다.
